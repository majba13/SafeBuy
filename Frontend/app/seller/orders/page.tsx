...
interface SellerOrder {
  ...
  orderNumber: string;
  buyer: { name: string; email: string };
  items: { product: { _id: string; title: string }; quantity: number; price: number; status: string }[];
  totalAmount: number;
  status: string;
  paymentStatus: string;
  shippingAddress: { fullName: string; phone: string; city: string; district: string };
          ...
          <div key={order._id} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-orange-600 mb-3">৳{order.totalAmount.toLocaleString()}</p>
                {order.status !== 'delivered' && order.status !== 'cancelled' && (
                  <select
                    value={order.status}
                    onChange={(e) => updateStatus(order._id, e.target.value)}
                    disabled={updating === order._id}
                    className="border border-gray-300 text-xs rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white"
                  >
                    {STATUS_OPTIONS.map((s) => <option key={s} value={s} className="capitalize">{s}</option>)}
                  </select>
                )}
